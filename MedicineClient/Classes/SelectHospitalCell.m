...
@implementation SelectHospitalCell
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        [self setIcon:[UIImage imageNamed:@"vip_sex_normal"] editedIcon:[UIImage imageNamed:@"vip_sex_selected"] placeholder:@"定向范围"];
    }
...
- (void)layoutSubviews {
    ...
    [self.femaleButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.maleButton.mas_right).offset(50);
        make.width.equalTo(@100);
        make.height.equalTo(@30);
        make.centerY.equalTo(self.contentView.mas_centerY);
    }];
...
- (void)setSex:(NSString *)sex {
    self.maleButton.selected = [sex isEqualToString:khospMale];
    self.femaleButton.selected = [sex isEqualToString:khospiFemale];
...
- (UIButton *)maleButton {
    if (!_maleButton) {
        ...
        [_maleButton setTitle:@"   团队" forState:UIControlStateNormal];
        [_maleButton setTitleColor:DefaultBlackLightTextClor forState:UIControlStateNormal];
        [_maleButton setTitleColor:DefaultGrayLightTextClor forState:UIControlStateSelected];
    }
    return _maleButton;
}
...
- (UIButton *)femaleButton {
    if (!_femaleButton) {
        ...
        [_femaleButton setTitle:@"   医生" forState:UIControlStateNormal];
