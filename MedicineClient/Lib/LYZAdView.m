...
- (void)createUpdateView {
    ...
    {
        UIImage *img = [UIImage imageNamed:@"alert_bg"];
        // 创建信息窗体view
...
//        [self.messageView addSubview:textView];
        ...
        [updateBtn setImage:[UIImage imageNamed:@"auth_button"] forState:UIControlStateNormal];
        ...
        if (!message.auisforce) {
            ...
            [self.closeBtn setImage:[UIImage imageNamed:@"auth_close"] forState:UIControlStateNormal];
...
//    }
    ...
    auth.title = @"认证";
    [[RootManager sharedManager].tabbarController.selectedViewController pushViewController:auth animated:YES];
