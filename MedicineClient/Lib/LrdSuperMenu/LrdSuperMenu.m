...
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height {
    ...
    if (self) {
        ...
        _textColor = DefaultGrayTextClor;
        _selectedTextColor = DefaultBlackLightTextClor;
...
- (void)setDataSource:(id<LrdSuperMenuDataSource>)dataSource {
    ...
    for (int i = 0; i < _numberOfColumn; i++) {
        ...
        //indicatorLayer
        CGPoint indicatorPosition = CGPointMake(size.width + kScreenWidth/2 +25, self.height / 2);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!cell) {
        cell = [[CustomTableViewCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:ID];
        cell.label.textColor = _textColor;
        cell.label.highlightedTextColor = _selectedTextColor;
    ...
    if (tableView == _leftTableView) {
        if (_dataSourceFlag.titleForRowsAtIndexPath) {
            cell.label.text = [_dataSource menu:self titleForRowAtIndexPath:[LrdIndexPath indexPathWithColumn:_currentSelectedColumn row:indexPath.row]];
        ...
        if (_dataSourceFlag.titleForItemInRowAtIndexPath) {
            ...
            cell.label.text = [_dataSource menu:self titleForItemsInRowAtIndexPath:[LrdIndexPath indexPathWithColumn:_currentSelectedColumn row:currentSelectedRow item:indexPath.row]];
        ...
        if ([cell.label.text isEqualToString:[(CATextLayer *)_titles[_currentSelectedColumn] string]]) {
            NSInteger currentSelectedRow = [_currentSelectedRows[_currentSelectedColumn] integerValue];
